...
describe('Integration tests for the tasks API', () => {
	...
	describe('Successfull requests', () => {
		...
		it('GET /api/tasks/:taskID - success - get task by ID', async () => {
			const { body, statusCode } = await request(app).get(`/api/tasks/${validTaskID}`)
			expect(statusCode).toBe(200)
			expect(body).toEqual(
				expect.objectContaining({
					success: true,
	...
	describe('failed requests', () => {
		it('POST /api/tasks - failure - on invalid post body ', async () => {
		...
		it('GET /api/tasks/:taskID - failure - on invalid taskID', async () => {
			const { body, statusCode } = await request(app).get(`/api/tasks/${invalidTaskID}`)
			expect(statusCode).toBe(400)
			expect(body).toEqual(expect.objectContaining({ success: false, message: 'invalid  taskID.' }))
		})
		it('PATCH /api/tasks/:taskID - failure - on invalid patch body', async () => {
			const invalidPatchBody = {}
			const { body, statusCode } = await request(app)
				.patch(`/api/tasks/${validTaskID}`)
				.set('Authorization', bearerToken)
				.send(invalidPatchBody)
			expect(statusCode).toBe(400)
			expect(body).toEqual(
				expect.objectContaining({ success: false, message: 'at least one task field is required' }),
			)
		})
		it('DELETE /api/tasks/:taskID - failure - on invalid taskID', async () => {
			const { body, statusCode } = await request(app)
				.patch(`/api/tasks/${invalidTaskID}`)
				.set('Authorization', bearerToken)
			expect(statusCode).toBe(400)
			expect(body).toEqual(expect.objectContaining({ success: false, message: 'invalid task ID.' }))
		})
	})
	//on failure
})
